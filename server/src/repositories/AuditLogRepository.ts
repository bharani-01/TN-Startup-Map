...
export class AuditLogRepository {
  ...
  async recordLog(data: Omit<ApiAccessLog, 'id' | 'createdAt'>): Promise<void> {
    ...
    // 2. Asynchronously persist to PostgreSQL without blocking HTTP response
    try {
      await (prisma as any).apiAccessLog.create({
        data: {
          id,
      ...
      // Non-fatal, suppress to not affect business API calls
      console.error('[AuditLog] Error recording API log to database:', err);
    }
  }

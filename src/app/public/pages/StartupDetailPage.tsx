...
export const StartupDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchJobs = async () => {
      try {
        const res = await fetch(`/api/jobs/startup/${slug}`);
        const data = await res.json();
      ...
      <div className="relative z-10 max-w-[1720px] mx-auto px-4 sm:px-6 lg:px-10 xl:px-12 py-6 sm:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {/* Open Career Opportunities / Hiring Module */}
            {(startup.isHiring || openJobs.length > 0) && (
              <div className="space-y-4 pt-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs font-mono font-extrabold uppercase tracking-[0.2em] text-[#86868B]">
                    <Briefcase className="w-3.5 h-3.5 text-[#0071E3]" />
                    <span>Career Opportunities & Hiring</span>
                  </div>
                  {openJobs.length > 0 && (
                ...
                {openJobs.length > 0 ? (
                  <div className="grid grid-cols-1 gap-3 pt-1">
                    {openJobs.map((job) => (
                      <div
                        key={job.id}
                        className="p-4 rounded-2xl bg-white border border-black/[0.06] hover:border-[#0071E3]/30 transition-all shadow-2xs space-y-3"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <h4 className="font-bold text-sm text-[#1D1D1F]">{job.title}</h4>
                              <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-[#0071E3]/10 text-[#0071E3]">
                                {job.jobType?.replace('_', ' ')}
                              </span>
                              {job.isRemote && (
                                <span className="text-[10px] font-mono font-medium px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600">
                                  Remote
                                </span>
                              )}
                            </div>
                        ...
                        {job.skills && job.skills.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 pt-1">
                            {job.skills.map((skill: string, idx: number) => (
                              <span
                                key={idx}
                                className="px-2 py-0.5 rounded-md bg-black/[0.03] border border-black/[0.04] text-[10px] font-mono font-medium text-[#86868B]"
                              >
                                {skill}
                              </span>
                            ))}
                  ...
                  <div className="p-5 rounded-2xl bg-white border border-black/[0.06] text-center space-y-2">
                    <p className="text-xs font-semibold text-[#1D1D1F]">This venture is actively growing its team.</p>
                    <p className="text-[11px] text-[#86868B]">
                      Positions are opening regularly. You can reach out directly via their official inquiry channel.
                    </p>
                    {startup.contactEmail && (
                      <a
                        href={`mailto:${startup.contactEmail}?subject=Talent Inquiry - ${encodeURIComponent(startup.name)}`}
                        className="inline-flex items-center gap-1 text-xs font-semibold text-[#0071E3] hover:underline pt-1"
                      >
                        <Mail className="w-3.5 h-3.5" />
                        <span>Send Direct Talent Inquiry</span>
                      </a>
                    )}
                  </div>
                )}

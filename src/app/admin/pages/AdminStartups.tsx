...
import { Building2, Search, Trash2, ShieldCheck, Loader2, ExternalLink, RefreshCw, Archive, RotateCcw, Activity, Eye } from 'lucide-react';
...
export const AdminStartups: React.FC = () => {
  ...
  const fetchStartups = async () => {
    try {
      setLoading(true);
      const [activeRes, archivedRes, viewsRes] = await Promise.all([
        fetch('/api/startups?limit=5000'),
        fetch('/api/startups?includeDeleted=true&limit=5000'),
        fetch('/api/admin/analytics/startups-views', { headers: { Authorization: `Bearer ${token}` } }),
      ]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Manage live verified companies, track visitor views, and inspect non-destructive soft-deleted records.
          </p>
        </div>
              ...
              <tbody className="divide-y divide-white/5">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((startup) => {
                    const views = viewsMap[startup.id] || viewsMap[startup.slug] || 0;
                    return (
                      <tr key={startup.id} className="hover:bg-white/[0.03] transition-colors">
                        ...
                        <td className="py-4 px-4">
                          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-mono font-bold bg-[#0071E3]/15 text-sky-300 border border-[#0071E3]/30">
